...
export function createAuthRouter(authService: AuthService): Router {
  ...
  router.post('/login', (req, res) => {
    ...
    if (!authService.isLocked()) {
      res.status(400).json({ error: 'Auth is not enabled. Run `botforje-js lock` first.' })
      return
    }
    ...
    res.setHeader(
      'Set-Cookie',
      `botforje_js_session=${session.token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=86400`
    )
    res.json({ success: true, expiresAt: session.expiresAt })
  })

  router.post('/logout', (req, res) => {
    const token = parseCookie(req.headers.cookie, 'botforje_js_session')
    if (token) {
      authService.invalidateSession(token)
    }
    res.setHeader(
      'Set-Cookie',
      'botforje_js_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0'
    )
...
export function findSessionToken(authService: AuthService, cookieHeader: string | undefined): string | null {
  const token = parseCookie(cookieHeader, 'botforje_js_session')
